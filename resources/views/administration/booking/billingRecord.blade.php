@extends('master')
@section('title', 'Billing Record')
@section('breadcrumb_title', 'Billing Record')
@push('style')
<style scoped>
...
<div id="bookingRecord">
    <div class="row" style="margin:0;">
        <fieldset class="scheduler-border bg-of-skyblue">
            <legend class="scheduler-border">Search Billing Record</legend>
                        ...
                        <template v-for="(booking, index) in bookings">
                            <tr>
                                ...
                                <td style="text-align:center;">
                                    <a href="" title="Billing Invoice" v-bind:href="`/billing-invoice-print/${booking.id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    @if(userAction('u'))
                            ...
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:right;">
                                    Total: @{{ booking.total | decimal }}<br>
                                    Paid: @{{ booking.advance | decimal }}<br>
                                    Due: @{{ booking.due | decimal }}
                                </td>
                                <td></td>
                            </tr>
                        </template>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.recordType == 'without'" style="display:none;" :style="{display: filter.recordType == 'without'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(booking, index) in bookings">
                            ...
                            <td style="text-align:right;">@{{ booking.subtotal | decimal }}</td>
                            <td style="text-align:right;">@{{ booking.vatAmount | decimal }}</td>
                            <td style="text-align:right;">@{{ booking.discountAmount | decimal }}</td>
                            <td style="text-align:right;">@{{ booking.total | decimal }}</td>
                            <td style="text-align:right;">@{{ booking.advance | decimal }}</td>
                            <td style="text-align:right;">@{{ booking.due | decimal }}</td>
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.subtotal)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.vatAmount)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.discountAmount)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.total)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.advance)}, 0) | decimal }}</td>
                            <td style="text-align:right;">@{{ bookings.reduce((prev, curr)=>{return prev + parseFloat(curr.due)}, 0) | decimal }}</td>
                            <td></td>
                            <td></td>
                        </tr>
                    </tbody>
