...
@section('breadcrumb_title', 'Purchase List')
@push('style')
<style scoped>
...
<div id="purchaseList">
    <div class="row" style="margin:0;">
        <fieldset class="scheduler-border bg-of-skyblue">
            <legend class="scheduler-border">Search Purchase List</legend>
                    ...
                    <tbody>
                        <template v-for="(purchase, index) in purchases">
                            <tr>
                                ...
                                <td>@{{ purchase.purchase_details[0].asset.name }}</td>
                                <td style="text-align:center;">@{{ purchase.purchase_details[0].price | decimal }}</td>
                                <td style="text-align:center;">@{{ purchase.purchase_details[0].quantity | decimal }}</td>
                                <td style="text-align:right;">@{{ purchase.purchase_details[0].total | decimal }}</td>
                                <td style="text-align:center;">
                                    <a href="" title="Order Invoice" v-bind:href="`/purchase-invoice-print/${purchase.id}`" target="_blank"><i class="fa fa-file-text"></i></a>
                                    @if(userAction('u'))
                            ...
                            <tr v-for="(item, sl) in purchase.purchase_details.slice(1)">
                                <td colspan="4" :rowspan="purchase.purchase_details.length - 1" v-if="sl == 0"></td>
                            ...
                            <tr style="font-weight:bold;">
                                ...
                                <td style="text-align:center;">Total Quantity<br>@{{ purchase.purchase_details.reduce((prev, curr) => {return prev + parseFloat(curr.quantity)}, 0) | decimal }}</td>
                ...
                <table class="table table-bordered record-table table-condensed" v-if="filter.searchType != 'quantity' && filter.recordType == 'without'" style="display:none;" :style="{display: filter.searchType != 'quantity' && filter.recordType == 'without'? '': 'none'}">
                    ...
                    <tbody>
                        <tr v-for="(purchase, index) in purchases">
                            ...
                            <td>@{{ purchase.date | dateFormat("DD-MM-YYYY") }}</td>
                            <td>@{{ purchase.supplier.name }}</td>
                            <td style="text-align:right;">@{{ purchase.sub_total | decimal }}</td>
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">@{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.sub_total)}, 0) | decimal }}</td>
        ...
        methods: {
            ...
            async deletePurchase(row) {
                ...
                if (confirm("Are you sure !!")) {
                    axios.post("/delete-purchase", formdata)
                        .then(res => {
            ...
            async print() {
                ...
                let reportContent = `
					<div class="container">
                        ...
                        <div class="row">
							<div class="col-xs-6">
								${userText} ${supplierText} ${assetText} 
							</div>
