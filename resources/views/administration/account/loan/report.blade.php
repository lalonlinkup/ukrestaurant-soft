...
@section('breadcrumb_title', 'Loan Transaction Report')
@push('style')
<style scoped>
                ...
                <table class="table table-bordered table-condensed" id="transactionsTable">
                    ...
                    <tbody>
                        <tr v-for="(transaction, sl) in transactions">
                            ...
                            <td style="text-align: left;">@{{ transaction.loan_account.name }} - @{{ transaction.loan_account.number }} - @{{ transaction.loan_account.bank_name }}</td>
                            <td>@{{ transaction.type }}</td>
                            <td>@{{ transaction.note }}</td>
                            <td style="text-align:right">@{{ transaction.type == "Receive" || transaction.type == "Interest"? transaction.amount : 0 | decimal }}</td>
                        ...
                        <tr style="font-weight:bold;">
                            ...
                            <td style="text-align:right;">
                                @{{ transactions.reduce((prev, curr) => {
                                        return prev + parseFloat(curr.type == 'Receive' || curr.type == 'Interest' ? curr.amount : 0)
                                    }, 0) | decimal }}
                            </td>
                            <td style="text-align:right;">
                                @{{ transactions.reduce((prev, curr) => {
    ...
    var transactionReport = new Vue({
        ...
        computed: {
            computedAccounts() {
                ...
                return accounts.map(account => {
                    account.display_name = `${account.bank_name} - ${account.number}`;
                    return account;
                })
            }
        },
        ...
        methods: {
            ...
            getTransactions() {
                ...
                axios.post('/get-loan-transactions', this.filter)
                    .then(res => {
                        let r = res.data;
                        this.transactions = r;
            ...
            async print() {
                ...
                if (this.selectedAccount != null) {
                    accountText =
                        `<strong>Account: </strong> ${this.selectedAccount.name} - ${this.selectedAccount.code}<br>`;
                }
